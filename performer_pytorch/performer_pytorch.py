...
def softmax_kernel(data, *, projection_matrix, is_query, normalize_data=True, eps=1e-4, device=None):
    """Constructs nonnegative kernel features for fast softmax attention.
      Args:
        ...
        is_query: predicate indicating whether input data corresponds to queries or keys
...
class Performer(nn.Module):
    def __init__(self, dim, depth, heads, causal=False, ff_mult=4, nb_features=None, reversible=False, ff_chunks=1,
                 generalized_attention=False, kernel_fn=nn.ReLU(), qr_uniform_q=False, use_scalenorm=False,
                 use_rezero=False):
        super().__init__()
...
class PerformerLM(nn.Module):
    def __init__(self, *, num_tokens, max_seq_len, dim, depth, heads, causal=False, ff_mult=4, nb_features=None,
                 reversible=False, ff_chunks=1, generalized_attention=False, kernel_fn=nn.ReLU(), qr_uniform_q=False,
                 use_scalenorm=False, use_rezero=False, tie_embedding=False, remap_vocab=False):
        super().__init__()
        self.dim = dim # length of vocab size
        self.num_tokens = num_tokens # length of vocab size
        self.max_seq_len = max_seq_len
        self.token_emb = nn.Embedding(num_tokens, dim)
        self.pos_emb = nn.Embedding(max_seq_len, dim)
        self.remap_vocab = remap_vocab

        nn.init.normal_(self.token_emb.weight, std=0.02)
        nn.init.normal_(self.pos_emb.weight, std=0.02)

        self.performer = Performer(dim, depth, heads, causal, ff_mult, nb_features, reversible, ff_chunks,
                                   generalized_attention, kernel_fn, qr_uniform_q, use_scalenorm, use_rezero)
        self.norm = nn.LayerNorm(dim)
